...
linear_fig <- ggplot(posts, aes(x = (LogBiomassKGMean), y =(.prediction))) +
  ...
  geom_abline(slope = 1, intercept = 12.66, linetype = "dotted",  color = "red", 
              size = 1.5)+
  geom_abline(slope = 1.18, intercept = 12.66, linetype = "dashed", color = "red",
              size =1.5)+
  theme(legend.position = "none")+
  theme+
  ggtitle(bquote('Power relationship, slope=1.03-1.13, median'~R^2~'=0.98'))+
