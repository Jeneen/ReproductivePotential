...
fec <- readRDS("Output/reproductive_potential_serf/brms_fecundity_se.rds")
dat <- readRDS("Output/reproductive_potential_serf/alldata_SDandCIof1000_clean_cinner2020.rda")
...
max <- meds %>%
  ...
  ungroup()%>%
  slice_max(order_by=mean_fec, n=20)
min <- meds %>%
  group_by(Larger, Protection, LargerProtection, ReefCluster)%>%
  summarise(mean_fec = mean(median_pred),
            mean_biom = mean(median_biom))%>% 
  ungroup()%>%
  slice_min(order_by=mean_fec, n=20)
...
pdf(file = "Output/supplement/posterior_fec_vs_raw_biom.pdf",   # The directory you want to save the file in
