...
#Create new data for simulation - take reference level category and mean continuous vars
...
#select all countries/reef cluster/geographic basin
...
newd_unfished <- mutate(pred, Protection = "UnfishedHigh")
newd_unfished <- newd_unfished %>%
  mutate(ID  = row_number())

#predict for newdata fished area
posts_unfished <- newd_unfished %>% add_predicted_draws(fec,
                                                    re_formula = NULL,
                                                    category='response',
                                                    allow_new_levels = TRUE,
                                                    ndraws = 1000)
...
###################################################################################################################
####### ACCURATE REPRESENTATION OF GEOG BASIN UNFISHED ########
#now sample based on actual percentage protected in each region (based on Iain Caldwell data calcs)
...
file_names <- paste0("output/",
                     c("master_cent_indpac_serranidae.rda", 
                       "master_east_indpac_serranidae.rda", 
                       "master_trop_at_serranidae.rda", 
                       "master_west_indpac_serranidae.rda"))
...
#combine prediction dataframesxxxxxx

#load the dfs into environment
master_cent_indpac <- readRDS("output/master_cent_indpac_serranidae.rda")
master_east_indpac <- readRDS("output/master_east_indpac_serranidae.rda")
master_trop_at <- readRDS("output/master_trop_at_serranidae.rda")
master_west_indpac <- readRDS("output/master_west_indpac_serranidae.rda")

posts_30Unfished <- readRDS("output/medians_posts_30percentUnfished_serranidae.rda")
#posts_30Unfished <- unique(posts_30Unfished)
...
#get the percent difference in fecundity between each
#posts_gb
#posts_30Unfished
#posts_fished

#get median for posts_fished so same variation as other df
posts_fished_med <- TakeMedianDraws(posts_fished)


#get the percent differences between the median fecundity of each
...
posts_30GB_mean_ser <- posts_30GB %>% group_by(Geographic_Basin, Larger, ReefCluster, ID, label, orderFig) %>% 
 summarise(meanGB = mean(postsGB),
    mean30= mean(posts30),
    percent_diff = (((mean30 - meanGB)/meanGB)*100))

posts_30GB_mean_ser$percent_diff

#save
saveRDS(posts_30GB_mean_ser,  "output/fecundity_potential_GB_serranidae.rds")
