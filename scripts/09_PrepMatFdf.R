...
cinner <- read.csv("data/Cinneretal2020_multiplegoals_data.csv", header = T)
...
serf_all <- merge.data.frame(sex_lm_site, serf, by.x= c("genus_sp", "UniqueSite"), 
                             ...
                             all = TRUE)
missing <- filter(serf_all, is.na(lm_model))
missing <- as.data.frame(unique(missing$genus_sp))
names(missing) <- "genus_sp"
missing <- filter(serf_all, is.na(lm_model))
...
serf_dist <- uncount(serf_all, Number)
...
#merge with data
data <- merge.data.frame(matF, ab, by = c("genus_sp"), all=TRUE)
